...
│   └── Dockerfile
├── docs/
│   ├── requirement.md                    # 要件定義（システム構成図 / 機能要件 / 非機能要件）
│   ├── architecture.md                   # アーキテクチャ詳細（コンポーネント / 通信経路 / セキュリティ）
│   ├── deploy-guide.md                   # デプロイ手順（クイックスタート / Terraform / ACR）
│   ├── operations.md                     # 運用ガイド（監視 / トラブルシュート / 更新）
│   ├── cost-estimate.md                  # 月額コスト見積もり（リソース別 / 最適化ヒント）
│   ├── poc-vs-production.md              # PoC 前提と本番ベストプラクティスの差分
│   ├── custom-terminology-options.md     # 社内用語カスタマイズの実装オプション比較
│   └── architecture.drawio               # アーキテクチャ構成図（draw.io 形式）
├── infra/
│   ├── providers.tf
│   ├── main.tf
│   ├── variables.tf
