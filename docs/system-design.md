...
13. [運用ガイド](#13-運用ガイド)
14. [インフラ構成図（環境別詳細）](#14-インフラ構成図環境別詳細)
...
本セクションは [`infra/`](../infra/main.tf) 配下の Terraform 定義から生成した、Azure 構成の詳細図解です。
`dev` / `staging` / `prod` のすべての環境で同一トポロジーを採用しているため、図はトポロジー共通として 1 セット示し、環境ごとに変わるのは **リソース名のサフィックス** と **タグ** のみです（[14.5 環境別リソース名一覧](#145-環境別リソース名一覧) を参照）。
...
### 14.2 ネットワーク構成（VNet / Subnet / Private Endpoint）
...
### 14.3 ID と RBAC（Managed Identity と権限割り当て）
