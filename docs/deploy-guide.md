...
### 2.5 Foundry エージェント登録（自動）

`postdeploy` フックにより、`azd deploy` / `azd up` の完了時に自動実行されます。

- `register_foundry_agents.py` を呼び出し、3 つの Prompt Agent を登録/更新
- 冪等（`create_version` による upsert）
- `FOUNDRY_PROJECT_ENDPOINT` が azd env に設定されている場合のみ実行
...
`postdeploy` フックにより、`azd deploy` / `azd up` の完了時に `backend/app/data/terminology.json` が Blob Storage に自動アップロードされます。
...
```bash
# パブリックアクセスを一時有効化
az acr update --name acrmtgminutesdev --public-network-enabled true --default-action Allow
...
# パブリックアクセスを無効化（復元）
az acr update --name acrmtgminutesdev --public-network-enabled false --default-action Deny
```
